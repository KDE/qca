    ...
    bool endVerify(const SecureArray &sig)
    {
        if (state == VerifyActive) {
            if (raw_type) {
                ...
                if (type == EVP_PKEY_RSA) {
                    RSA *rsa = EVP_PKEY_get0_RSA(pkey);
                    if ((len = RSA_public_decrypt(sig.size(), (unsigned char *)sig.data(), (unsigned char *)out.data(),
...
public:
    ...
    ~RSAKey() override { delete keymaker; }

    Provider::Context *clone() const override { return new RSAKey(*this); }

    bool isNull() const override { return (evp.pkey ? false : true); }

    PKey::Type type() const override { return PKey::RSA; }

    bool isPrivate() const override { return sec; }
    ...
    int bits() const override { return EVP_PKEY_bits(evp.pkey); }
    ...
    SecureArray encrypt(const SecureArray &in, EncryptionAlgorithm alg) override
    {
        ...
        int ret;
        if (isPrivate())
            ret = RSA_private_encrypt(buf.size(), (unsigned char *)buf.data(), (unsigned char *)result.data(), rsa,
                                      pad);
        else
    ...
    void update(const MemoryRegion &in) override { evp.update(in); }

    QByteArray endSign() override { return evp.endSign().toByteArray(); }

    bool endVerify(const QByteArray &sig) override { return evp.endVerify(sig); }
...
public:
    ...
    ~DSAKey() override { delete keymaker; }

    Provider::Context *clone() const override { return new DSAKey(*this); }

    bool isNull() const override { return (evp.pkey ? false : true); }

    PKey::Type type() const override { return PKey::DSA; }
...
public:
    ...
    ~DHKey() override { delete keymaker; }

    Provider::Context *clone() const override { return new DHKey(*this); }

    bool isNull() const override { return (evp.pkey ? false : true); }

    PKey::Type type() const override { return PKey::DH; }
...
public:
    ...
    MyPKeyContext(Provider *p) : PKeyContext(p) { k = nullptr; }

    ~MyPKeyContext() override { delete k; }
    ...
    PKeyBase *key() override { return k; }

    const PKeyBase *key() const override { return k; }

    void setKey(PKeyBase *key) override { k = key; }
...
public:
    ...
    bool isNull() const { return (!cert && !req && !crl); }
...
public:
    ...
    Provider::Context *clone() const override { return new MyCertContext(*this); }

    QByteArray toDER() const override { return item.toDER(); }

    QString toPEM() const override { return item.toPEM(); }
...
public:
    ...
    MyCAContext(Provider *p) : CAContext(p) { privateKey = nullptr; }
    ...
    ~MyCAContext() override { delete privateKey; }
    ...
    Provider::Context *clone() const override { return new MyCAContext(*this); }
};
...
public:
    ...
    Provider::Context *clone() const override { return new MyCSRContext(*this); }
    ...
    bool createRequest(const CertificateOptions &opts, const PKeyContext &priv) override
    {
        ...
        if (!cs.isEmpty())
            X509_REQ_add1_attr_by_NID(x, NID_pkcs9_challengePassword, MBSTRING_UTF8, (const unsigned char *)cs.data(),
                                      -1);
    ...
    const CertContextProps *props() const override { return &_props; }
    ...
    QString toSPKAC() const override { return QString(); }
...
public:
    ...
    Provider::Context *clone() const override { return new MyCRLContext(*this); }
    ...
    const CRLContextProps *props() const override { return &_props; }
...
public:
    ...
    QStringList supportedCipherSuites(const TLS::Version &version) const override
    {
        ...
        if ((version >= TLS::TLS_vMIN && version <= TLS::TLS_vMAX) || version == TLS::TLS_v1) {
            static struct
            {
                TLS::Version ver;
                int          ssl_ver;
            } limits[] = { { TLS::TLS_v1, TLS1_VERSION },
    ...
    void setTrustedCertificates(const CertificateCollection &_trusted) override { trusted = _trusted; }
    ...
    void shutdown() override { mode = Closing; }
    ...
    bool eof() const override { return v_eof; }
    ...
    Validity peerCertificateValidity() const override { return vr; }
